...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'mftn06.mxy', email: 'knnrmi.oai@qq.com' };
  ...
  describe('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('h6lprj_g6y78');
      cy.get(`[data-cy="login"]`).should('have.value', 'h6lprj_g6y78');

      cy.get(`[data-cy="firstName"]`).type('靖琪');
      cy.get(`[data-cy="firstName"]`).should('have.value', '靖琪');

      cy.get(`[data-cy="lastName"]`).type('齐');
      cy.get(`[data-cy="lastName"]`).should('have.value', '齐');

      cy.get(`[data-cy="email"]`).type('g7u.it6@163.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'g7u.it6@163.com');

      cy.get(`[data-cy="langKey"]`).select('zh-cn');
